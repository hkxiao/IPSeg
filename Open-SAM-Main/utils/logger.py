...
class AverageMeter:
    ...
    def __init__(self, benchmark):
        ...
        if 'fold' in self.benchmark:
            self.class_ids_interest = [c for c in range(80) if c % int(dataset[-1]) == 0]
        elif 'perseg' in self.benchmark:
            self.class_ids_interest = range(40)
        elif 'fss' in self.benchmark:
            self.class_ids_interest = range(520, 760)
    ...
    def update(self, inter_b, union_b, class_id, loss):
        # print(self.intersection_buf.shape)
        # print(class_id, inter_b.shape)
    ...
    def compute_iou(self):
        ...
        # print(self.intersection_buf.shape, torch.stack([self.union_buf, self.ones]).shape)
        # print(self.class_ids_interest.shape)      
        # raise NameError
