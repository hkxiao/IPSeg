...
def get_arguments():
    ...
    parser.add_argument('--ref_img', action='store_true')
...
def persam(args, obj_name, images_path,  output_path):
    ...
    if args.ref_txt: ref_name = args.ref_idx
    elif args.ref_img: ref_name = "ref_" + str(obj_name)
    else: ref_name = sorted(os.listdir(os.path.join(images_path, obj_name)))[args.ref_idx][:-4]
    print(ref_name)
    
    if not args.ref_img:
        ref_feat_path = os.path.join(images_path.replace("imgs",'sd_raw+dino_feat'), obj_name, str(ref_name) + '.pth')
    else:
        ref_feat_path = os.path.join(images_path.replace("imgs",'sd_raw+dino_feat'),  str(ref_name) + '.pth')
